...
@Component({
  selector: 'app-porautorizar',
  templateUrl: './porautorizar.component.html',
  styleUrls: ['./porautorizar.component.css'],
...
this.primerParametro = this.parametro.parametro;
  ...
  getCursosAdmin() {
    this.paginacion = [];
    this.cuantos = 0;
    this._cursosService.getRegistrados(this.primerParametro).subscribe(
      response => {
        if (response.status === "success") {
          this.responsablesp = response.cursos;

                    for (let nombrecu of this.responsablesp) {
this.nombrecirsop=nombrecu.NOMBRECURSO;
break;
                      }
  ...
  getEventos1p() {
    ...
    this._cursosService.getRegistradosP(this.primerParametro,this.parametronom.value).subscribe(
