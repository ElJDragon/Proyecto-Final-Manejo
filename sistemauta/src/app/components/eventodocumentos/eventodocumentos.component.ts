...
import { EventoDocumento } from '../../models/EventoDocumento';
import { UserService } from '../../services/user.service';
import { DocumentosService } from '../../services/documentos.service';
import { EventosService } from '../../services/eventos.service';

@Component({
  selector: 'app-eventodocumentos',
  templateUrl: './eventodocumentos.component.html',
  styleUrls: ['./eventodocumentos.component.css'],
      providers: [UserService,DocumentosService,EventosService]
})
export class EventodocumentosComponent {
...
public evedocud: EventoDocumento;
...
this.evedocud= new EventoDocumento('','','');
  }
...
//TIPOS DOCUMENTO
 getTiposDocumentos($parametro: any)
  {
    this.paginacion=[];
        this.cuantos=0;
    this._documentosService.getParametro($parametro).subscribe(
response => {
...
this.registrototal=this.documentosp.length;
this.totaldepaginas=this.documentosp.length/this.registrosxpagina;
...
for (let numero of this.documentosp){
       if(this.registrototal>=this.registrosxpagina)
{
...
{
        if(this.cuantos<this.documentosp.length)
        {
      this.paginacion[this.cuantos] = numero;
      this.cuantos++;
        }
...
EventoRequerido(event: Event, $documento: any) {
  const checkbox = event.target as HTMLInputElement;
  this.requeridop = checkbox.checked ? '1' : '0';
   if(this.requeridop==1)
   {
this.guardarDocumentos(this.codigoEventop,$documento);
   }
   else
   {
this.eliminarDocumento(this.codigoEventop,$documento);
   }
}



guardarDocumentos($evento: any, $documento: any){
 
  this.evedocud= new EventoDocumento('',$evento,$documento);
this._documentosService.guardarEveDocumento(this.evedocud).subscribe(
response => {
if(response)
{
this.status= 'success';
...
console.log(<any>error);}          
   ...
   eliminarDocumento($evento: any, $documento: any){
  this._documentosService.deleteEveDocumento(this.token,$evento,$documento).subscribe(
...
{
this.getTiposDocumentos(this.codigoEventop);
this.isbusqueda=false;
}
}  
...
//este metodo recibe la respuesta y el error
  ...
  this._documentosService.deleteDocumento(this.token,this.codigoeliminar).subscribe(
//aqui recibo lo que el api me responde
